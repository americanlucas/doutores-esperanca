	...
	return (
		<div className={`flex-c rounded-xl px-6 py-4 gap-2 border-2 ${borderColorClass[props.status]}`}>
			...
			<div className="flex-r items-center justify-between">
                ...
                {props.status === "enviado" && <CardBadge titulo="Enviado" color="verde" />}
                {props.status === "revisão" && <CardBadge titulo="Revisão" color="cinza" />}
                {props.status === "pendente" && <CardBadge titulo="Pendente" color="cinza" />}
			</div>
